...
**Confidence intervals cover resampling, not seeds.** The $36,824 incremental-vs-baseline
figure carries a paired bootstrap interval over cases ($10,393–$62,276), which is why the
case-rate lift is the number to quote. It does **not** yet vary the simulator seed, so it
captures sampling variation within one world, not variation between worlds.
...
**No authentication or multi-tenancy.** The API is open and single-tenant. Fine for an
evaluated artifact, not for a deployment.

**Next**: replicate across simulator seeds; a real sandbox adapter for one rail; per-arm
holdout so incremental lift is measured within surface rather than pooled; and a live Claude
A/B (`--planner llm`) to test whether the LLM beats the rules planner on the cases where the
error code lies.
